...
export default function NginxVhostTerminal({ onComplete }: { onComplete?: () => void }) {
    ...
    const [vhostLinked, setVhostLinked] = useState(false);
    const [nginxReloaded, setNginxReloaded] = useState(false);
    ...
    const tutorials = [
        {
            ...
            instruction: "Sama halnya dengan Apache, Nginx menyimpan Server Blocks (sebutan Nginx untuk VHost) di /etc/nginx/sites-available/.",
            task: "Ketik `cd /etc/nginx/sites-available/`.",
            check: (cmd: string, states: any) => states.cwd.join('/') === 'etc/nginx/sites-available'
        },
        {
            title: "Buat File Konfigurasi",
            instruction: "Gunakan editor file teks console untuk membuat file pengaturan web baru.",
            task: "Ketik `sudo nano web1.conf`.",
            check: (cmd: string) => cmd.trim() === 'sudo nano web1.conf' || cmd.trim() === 'nano web1.conf'
        },
        {
            title: "Isi Server Block & Simpan",
            instruction: "Ketik konfigurasi dasar nginx:\nserver {\n  listen 80;\n  server_name web1.com;\n  root /var/www/html/web1;\n}",
            task: "Ketik isi file, simpan (Ctrl+O Enter), lalu keluar (Ctrl+X).",
            check: (cmd: string, states: any) => states.vhostCreated
        },
        {
            title: "Aktifkan Server Block",
            instruction: "Berbeda dengan Apache yang punya tool a2ensite, di Nginx kita cukup membuat shortcut (symlink) dari sites-available ke folder sites-enabled.",
            task: "Ketik `sudo ln -s /etc/nginx/sites-available/web1.conf /etc/nginx/sites-enabled/`.",
            check: (cmd: string) => cmd.trim() === 'sudo ln -s /etc/nginx/sites-available/web1.conf /etc/nginx/sites-enabled/' || cmd.trim() === 'ln -s /etc/nginx/sites-available/web1.conf /etc/nginx/sites-enabled/'
        },
        {
            title: "Reload Nginx",
            instruction: "Terapkan perubahan konfigurasi dengan me-reload kembali Nginx.",
            task: "Ketik `sudo systemctl reload nginx`.",
            check: (cmd: string) => cmd.trim() === 'sudo systemctl reload nginx' || cmd.trim() === 'systemctl reload nginx'
        },
        {
            title: "Selesai",
            instruction: "Selamat! Pemahaman dasar routing HTTP untuk berbagai server sudah kamu kuasai.",
            task: "Ketik `exit` untuk mengakhiri sesi Simulasi Instalasi Industri server ini.",
    ...
    useEffect(() => {
        if (output.length === 0) {
            setOutput([
                ...
                { type: "response", content: <span className="text-white/60">Modul 11: Nginx Server Blocks loaded...</span> },
    ...
    const handleCommand = async (e: React.FormEvent) => {
        ...
        switch (mainCmd) {
            case 'help':
                response = "Perintah simulasi tersedia: cd, sudo, nano, ln, systemctl, ls, clear, exit";
                break;
            case 'clear':
                setOutput([]);
                return;
            case 'cd':
                const path = args[1];
                if (path === '/etc/nginx/sites-available' || path === '/etc/nginx/sites-available/') {
                    setCwd(['etc', 'nginx', 'sites-available']);
                } else if (!path || path === '~') {
                    setCwd(['home', 'cadet']);
                } else {
            ...
            case 'systemctl':
                ...
                if (actionCmd === 'nano') {
                    if (!actionSub) {
                        ...
                        setEditorContent(vhostCreated && actionSub === 'web1.conf' ? "server {\n  listen 80;\n  server_name web1.com;\n  root /var/www/html/web1;\n}" : "");
                        setIsEditorOpen(true);
                        setTimeout(() => { if (tutorials[step]?.check(cmd, {})) completeStep(); }, 100);
                    }
                } else if (actionCmd === 'ln' && args.includes('-s') && args.includes('/etc/nginx/sites-available/web1.conf') && args.includes('/etc/nginx/sites-enabled/')) {
                    if (!vhostCreated) {
                        newOutputLines = [{ type: 'response', content: 'ln: failed to create symbolic link /etc/nginx/sites-enabled/web1.conf: No such file or directory /etc/nginx/sites-available/web1.conf' }];
                    } else {
                        newOutputLines = []; // success usually silent in ln
                        setVhostLinked(true);
                        setTimeout(() => { if (tutorials[step]?.check(cmd, {})) completeStep(); }, 100);
                    }
                } else if (actionCmd === 'systemctl') {
                    if (actionSub === 'reload' && args.includes('nginx')) {
                        if (!vhostLinked) {
                            newOutputLines = [{ type: 'response', content: 'nginx reloaded (nothing changed).' }];
                        } else {
                            newOutputLines = []; // usually empty on success
                            setNginxReloaded(true);
                            setTimeout(() => { if (tutorials[step]?.check(cmd, {})) completeStep(); }, 100);
                        }
                    } else {
                ...
                break;
            case 'ls':
                if (cwd.join('/') === 'etc/nginx/sites-available') {
                    let list = "default";
            ...
            case 'exit':
                if (step === tutorials.length - 1) {
                    ...
                    window.location.href = "/play"; // Back to main CTF page, since this is the last session
                } else {
        ...
        <div className={`flex flex-col bg-black border border-white/10 rounded-lg overflow-hidden font-mono shadow-2xl transition-all duration-300 ${isMaximized ? "fixed inset-0 z-50 rounded-none border-0" : "w-full h-[600px]"}`}>
            ...
            <div className="flex items-center justify-between px-4 py-2 bg-[#1a1a1c] border-b border-white/5 select-none shrink-0">
                <div className="flex items-center gap-2 text-white/60">
                    ...
                    <span className="text-xs font-bold">cadet@ctf:{cwd[cwd.length - 1] === 'cadet' ? '~' : `/${cwd.join('/')}`}</span>
                </div>
