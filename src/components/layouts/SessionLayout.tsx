    ...
    return (
        <div className="flex h-screen bg-background text-foreground overflow-hidden font-mono">
            ...
            <main className="flex-1 flex flex-col relative overflow-hidden">
                {/* Header */}
                <header className="h-14 border-b border-white/10 bg-[#0a0a0a]/50 backdrop-blur-md flex items-center justify-between px-6 shrink-0 z-10 relative">
                    <div className="flex items-center gap-4">
                        <h1 className="text-lg font-bold text-[#E95420] tracking-tight flex items-center gap-2">
                            <span className="w-2 h-2 rounded-full bg-[#E95420] animate-pulse" />
                            {title}
                        </h1>
                    </div>
                    <div className="text-xs text-white/40 font-mono uppercase tracking-widest hidden sm:block">
                        {subtitle}
                    </div>
                </header>
                ...
                <div className="flex-1 overflow-hidden p-6 relative flex flex-col">
                    ...
                    <div className="w-full max-w-6xl mx-auto h-full flex flex-col gap-6 relative z-10">
                        ...
                        {showObjectives && objectives.length > 0 && (
                            <div className="shrink-0 bg-[#0a0a0a] border border-white/10 rounded-lg p-4 flex items-start gap-4 text-xs text-white/60 shadow-lg">
                                <div className="bg-[#E95420]/10 p-2 rounded text-[#E95420] shrink-0">
                                    <Info size={16} />
                                </div>
                                <div className="flex-1">
                                    <h4 className="font-bold text-white mb-2">Tujuan Pembelajaran</h4>
                                    <ul className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-1 list-disc list-inside opacity-80">
