...
import { Level6Terminal } from '@/components/Level6Terminal';
...
export default function Level6() {
    ...
    const { submitFlag, status, alreadyCompleted, setStatus } = useSubmitFlag(6, 'yadika{ps_aux_grep}');
    ...
    useEffect(() => {
        if (!loading) {
            if (!user) router.push('/');
            else if (!isLevelUnlocked(6)) router.push('/play/5');
        }
