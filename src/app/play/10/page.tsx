...
import { Level10Terminal } from '@/components/Level10Terminal';
...
export default function Level10() {
    ...
    const { submitFlag, status, alreadyCompleted, setStatus } = useSubmitFlag(10, 'yadika{bash_script_hero}');
    ...
    useEffect(() => {
        if (!loading) {
            if (!user) router.push('/');
            else if (!isLevelUnlocked(10)) router.push('/play/9');
        }
    ...
    return (
        <SessionLayout title="Level 10: Bash Script Runner" currentLevel={10} showObjectives={false}>
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="px-3 md:px-4 py-2 md:py-3 border-b border-white/5 flex items-center justify-between">
                    <p className="text-[10px] md:text-xs text-foreground/50 font-mono flex-1 mr-4">
                        Jalankan script dengan parameter yang benar untuk mendapatkan flag. Format: <code className="text-primary">yadika{'{...}'}</code>
                    </p>
                ...
                <div className="flex-1 p-3 md:p-4 overflow-hidden">
                    <Level10Terminal onFlagFound={handleFlagFound} />
                </div>
                ...
                <div className="px-3 md:px-4 py-2 md:py-3 border-t border-white/5">
                    ...
                    <form onSubmit={handleSubmit} className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-black/50 border border-white/10 rounded px-2 md:px-3 py-1.5 md:py-2 font-mono text-xs md:text-sm text-terminal focus:border-primary outline-none disabled:opacity-50"
                        />
                        <button
                            type="submit"
