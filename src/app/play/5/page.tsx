...
export default function Level5() {
    ...
    useEffect(() => {
        if (!loading) {
            if (!user) router.push('/');
            else if (!isLevelUnlocked(5)) router.push('/play/4');
        }
    ...
    return (
        <SessionLayout title="Level 5: Strict Rules" currentLevel={5} showObjectives={false}>
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="px-3 md:px-4 py-2 md:py-3 border-b border-white/5 flex items-center justify-between">
                    <p className="text-[10px] md:text-xs text-foreground/50 font-mono flex-1 mr-4">
                        Kelola file permissions dengan chmod untuk mendapatkan FLAG. Format: <code className="text-orange-400">yadika{'{...}'}</code>
                    </p>
                ...
                <div className="px-3 md:px-4 py-2 md:py-3 border-t border-white/5">
                    ...
                    <form onSubmit={handleSubmit} className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-black/50 border border-white/10 rounded px-2 md:px-3 py-1.5 md:py-2 font-mono text-xs md:text-sm text-orange-400 focus:border-orange-500 outline-none disabled:opacity-50"
                        />
                        <button
                            type="submit"
                            disabled={status === 'correct' || status === 'submitting'}
                            className={`px-3 md:px-4 py-1.5 md:py-2 rounded font-bold text-xs md:text-sm flex items-center gap-1 md:gap-1.5 transition-all ${status === 'correct' ? 'bg-green-500 text-black' : 'bg-orange-500 text-white hover:bg-orange-400'
                                }`}
                        >
                            {status === 'correct' ? (
