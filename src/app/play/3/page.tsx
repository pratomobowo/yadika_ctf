                ...
                <div className="px-3 md:px-4 py-2 md:py-3 border-t border-white/5">
                    ...
                    <form onSubmit={handleSubmit} className="flex gap-2">
                        <input
                            ...
                            className="flex-1 bg-black/50 border border-white/10 rounded px-2 md:px-3 py-1.5 md:py-2 font-mono text-xs md:text-sm text-accent focus:border-accent outline-none disabled:opacity-50"
                        />
                        <button
                            type="submit"
                            disabled={status === 'correct' || status === 'submitting'}
                            className={`px-3 md:px-4 py-1.5 md:py-2 rounded font-bold text-xs md:text-sm flex items-center gap-1 md:gap-1.5 transition-all ${status === 'correct' ? 'bg-green-500 text-black' : 'bg-accent text-white hover:bg-accent/80'
                                }`}
                        >
                            {status === 'correct' ? (
