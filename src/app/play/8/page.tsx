...
import { Level8Terminal } from '@/components/Level8Terminal';
...
export default function Level8() {
    ...
    const { submitFlag, status, alreadyCompleted, setStatus } = useSubmitFlag(8, 'yadika{env_var_found}');
    ...
    useEffect(() => {
        if (!loading) {
            if (!user) router.push('/');
            else if (!isLevelUnlocked(8)) router.push('/play/7');
        }
    ...
    return (
        <SessionLayout title="Level 8: Environment Secrets" currentLevel={8} showObjectives={false}>
            <div className="flex-1 flex flex-col overflow-hidden">
                <div className="px-4 py-3 border-b border-white/5 flex items-center justify-between">
                    <p className="text-xs text-foreground/50 font-mono">
                        Gunakan printenv untuk melihat variabel lingkungan. Format: <code className="text-primary">yadika{'{...}'}</code>
                    </p>
                ...
                <div className="flex-1 p-4 overflow-hidden">
                    <Level8Terminal onFlagFound={handleFlagFound} />
                </div>
